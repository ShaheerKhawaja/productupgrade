...
This is the Codex-native workflow wrapper for [.claude/commands/plan-ceo-review.md](../../.claude/commands/plan-ceo-review.md).

Use it when the user wants this exact ProductionOS workflow, not just the umbrella `productionos` router.

## Source of Truth

1. Read the source command spec at [.claude/commands/plan-ceo-review.md](../../.claude/commands/plan-ceo-review.md).
...
- Expected behavior: Challenge scope, tighten user value, and surface expansion opportunities explicitly.
- Validation: tests/runtime-targets.test.ts

## Inputs

- `mode` — Review mode: expansion | selective | hold | reduction (default: selective) Default: `selective` Optional.
- `target` — Plan, feature, or codebase to review Optional.
