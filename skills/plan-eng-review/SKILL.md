...
This is the Codex-native workflow wrapper for [.claude/commands/plan-eng-review.md](../../.claude/commands/plan-eng-review.md).

Use it when the user wants this exact ProductionOS workflow, not just the umbrella `productionos` router.

## Source of Truth

1. Read the source command spec at [.claude/commands/plan-eng-review.md](../../.claude/commands/plan-eng-review.md).
...
- Expected behavior: Lock architecture, trust boundaries, error paths, and test coverage before implementation.
- Validation: tests/runtime-targets.test.ts

## Inputs

- `target` — Plan or architecture to review (default: current work) Optional.

## Execution Outline

1. Preamble
2. Restate the Execution Target

## Agents And Assets

- Agents: `architecture-designer`, `database-auditor`, `vulnerability-explorer`
